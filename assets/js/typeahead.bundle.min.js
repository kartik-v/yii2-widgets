...
!function (a) {
	var b = {isMsie: function () {
		...
		function c(b) {
			b = b || {}, this._send = b.send ? e(b.send) : a.ajax, this._get = b.rateLimiter ? b.rateLimiter(this._get) : this._get
		}
		...
		return b.mixin(c.prototype, {bootstrap: function (a) {
			...
			return c = d(this.queryTokenizer(a)), b.each(c, function (a) {
				...
				return b && 0 === c.length ? (f = b.ids.slice(0), e = e ? g(e, f) : f, void 0) : (e = [], !1)
		...
		function e(d) {
			var e, f;
			return f = {url: null, thumbprint: "", ttl: 864e5, filter: null, ajax: {}}, (e = d.prefetch || null) && (e = b.isString(e) ? {url: e} : e, e = b.mixin(f, e), e.thumbprint = c + e.thumbprint, e.ajax.method = e.ajax.method || "get", e.ajax.dataType = e.ajax.dataType || "json", !e.url && a.error("prefetch requires url to be set")), e
		}
		...
		return j = {data: "data", protocol: "protocol", thumbprint: "thumbprint"}, c.tokenizers = {whitespace: function (a) {
			...
			var b = {};
			return this.storage && (b.data = this.storage.get(j.data), b.protocol = this.storage.get(j.protocol), b.thumbprint = this.storage.get(j.thumbprint)), isExpired = b.thumbprint !== a || b.protocol !== location.protocol, b.data && !isExpired ? b.data : null
		}, initialize: function () {
			function b() {
				d.add(d.local)
			}
			...
			var e, f = this, g = !1;
			e = this.index.get(a).sort(this.sorter).slice(0, this.limit), e.length < this.limit && this.transport && (g = this._getFromRemote(a, d)), !g && c && c(e)
		}, ttAdapter: function () {
	...
	var k = function () {
		...
		function c(c) {
			c = c || {}, c.templates = c.templates || {}, c.source || a.error("missing source"), this.query = null, this.highlight = !!c.highlight, this.name = c.name || b.getUniqueId(), this.source = c.source, this.valueKey = c.displayKey || "value", this.templates = d(c.templates, this.valueKey), this.$el = a(i.dataset.replace("%CLASS%", this.name))
		}
		...
		var e = "ttDataset", f = "ttValue", g = "ttDatum";
		return c.extractDatasetName = function (b) {
			...
			function h() {
				return p.templates.empty({query: c})
			}

			function k() {
				function h(b) {
					var c, d, h;
					return d = p.templates.suggestion(b), h = i.suggestion.replace("%BODY%", d), c = a(h).data(e, p.name).data(f, b[p.valueKey]).data(g, b), c.children().each(function () {
						a(this).css(j.suggestionChild)
					}), c
				}

				var k;
				return k = a(i.suggestions).css(j.suggestions).append(b.map(d, h)), p.highlight && m({node: k[0], pattern: c}), k
			}
			...
			if (this.$el) {
				var o, p = this;
				this.$el.empty(), o = d && d.length, !o && this.templates.empty ? this.$el.html(h()).prepend(p.templates.header ? l() : null).append(p.templates.footer ? n() : null) : o && this.$el.html(k()).prepend(p.templates.header ? l() : null).append(p.templates.footer ? n() : null), this.trigger("rendered")
			}
		}, getRoot: function () {
			return this.$el
		}, update: function (a) {
		...
		return b.mixin(c.prototype, l, {_onMouseEnter: function () {
			...
			var b, c, d, e;
			if (this.isOpen) {
				if (c = this._getCursor(), b = this._getSuggestions(), this._removeCursor(), d = b.index(c) + a, d = (d + 1) % (b.length + 1) - 1, -1 === d)return this.trigger("cursorRemoved"), void 0;
			...
			this.isOpen && (this.isOpen = this.isMouseOverDropdown = !1, this._removeCursor(), this._hide(), this.trigger("closed"))
		}, open: function () {
		...
		function d(b, c) {
			var d, f, h, k;
			d = a(b), f = a(i.wrapper).css(j.wrapper), h = a(i.dropdown).css(j.dropdown), k = d.clone().css(j.hint).css(e(d)), k.removeData().addClass("tt-hint").removeAttr("id name placeholder").prop("disabled", !0).attr({autocomplete: "off", spellcheck: "false"}), d.data(g, {dir: d.attr("dir"), autocomplete: d.attr("autocomplete"), spellcheck: d.attr("spellcheck"), style: d.attr("style")}), d.addClass("tt-input").attr({autocomplete: "off", spellcheck: !1}).css(c ? j.input : j.inputWithNoHint);
			try {
		...
		return b.mixin(c.prototype, {_onSuggestionClicked: function (a, b) {
			...
			this.input.clearHint(), this.input.setQuery(a.value), this.input.setInputValue(a.value, !0), this.dropdown.empty(), this._setLanguageDirection(), b.defer(b.bind(this.dropdown.close, this.dropdown)), this.eventBus.trigger("selected", a.raw, a.datasetName)
		}, open: function () {
			this.dropdown.open()
		}, close: function () {
			this.dropdown.close()
	...
	!function () {
		var c, d;
		c = "ttTypeahead", d = {initialize: function (d) {
			function e() {
				var e, g, h = a(this);
				b.each(f, function (a) {
					a.highlight = !!d.highlight
				}), g = new q({input: h, eventBus: e = new k({el: h}), withHint: b.isUndefined(d.hint) ? !0 : !!d.hint, minLength: d.minLength, autoselect: d.autoselect, datasets: f}), h.data(c, g)
			}

			var f = [].slice.call(arguments, 1);
			return d = d || {}, this.each(e)
		}, open: function () {
			function b() {
				var b, d = a(this);
				(b = d.data(c)) && b.open()
			...
			function f() {
				var b, d, e = a(this);
				return(b = e.data(c)) && (d = b.getQuery()), d
			}
			...
			function b() {
				var b, d = a(this);
				(b = d.data(c)) && (b.destroy(), d.removeData(c))
			}
